...
fun main () {
    ...
    val inputTest = getInputReader("/inputAoC6Test.txt")
...
fun createOrbitTree( input: InputStreamReader) : List<Planet> {
    ...
    input.readLines().forEach{
        ...
        val planetParent = planetsMap[parentName]?:Planet(parentName, mutableListOf())
        val planetChild = planetsMap[childName]?:Planet(childName,mutableListOf())
